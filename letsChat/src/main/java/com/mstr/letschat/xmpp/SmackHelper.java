...
import com.mstr.letschat.preferences.SharedPrefs;
...
public class SmackHelper {
    ...
    static String packetId = null;


    public static final String ACTION_CONNECTION_CHANGED = "com.dplit.sumsay.intent.action.CONNECTION_CHANGED";
    public static final String EXTRA_NAME_STATE = "com.dplit.sumsay.State";

    public SmackHelper(Context context) {
    ...
    public void sendChatMessage(String to, String body, PacketExtension packetExtension) throws SmackInvocationException, NotConnectedException {
    ...
    public void login(String username, String password) throws SmackInvocationException {
        ...
        try {
            if (!con.isAuthenticated()) {
                ...
                DeliveryReceiptManager.getInstanceFor(con).addReceiptReceivedListener(new ReceiptReceivedListener() {
                    @Override
                    public void onReceiptReceived(String arg0, String arg1, String arg2) {

                        //todo: code here for devivery recipts
                        Log.i(TAG, arg0 + ", " + arg1 + ", " + arg2);
                        new SharedPrefs(context).setDelivered_to(arg0);
                        Log.i("Saved in SahrePrefs", new SharedPrefs(context).getDelivered_to());

                        broadcastSent(arg0, arg1, arg2);
                    }
                });
            }
    ...
    String TAG = "SmackHelper";

    private void broadcastSent(String to, String from, String packet) {
        Intent intent = new Intent("intent_fiter");
        intent.putExtra("message_to", to);
        intent.putExtra("message_from", from);
        intent.putExtra("message_packet", packet);
        LocalBroadcastManager.getInstance(context).sendBroadcast(intent);
    }
